...
const OrangeMoney = () => {
  ...
  const handleCalculate = (e) => {
    ...
    axios
      .post(
        `https://orramo-backend2.herokuapp.com/api/calculate/eumoney/${values}/${value}`
      )
      .then((response) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.inputBox}>
        <Text style={{ fontSize: 12 }}>EUMoney</Text>
        <TextInput
          style={styles.input}
          value={values}
      ...
      <View style={styles.resultBox}>
        ...
        <View style={Calculated ? styles.resultComponent : null}>
          <View style={{ flexDirection: "row" }}>
            {Calculated ? (
              <Text style={styles.MainCharge}>EUMoney Charge : </Text>
            ) : null}
            <Text
              style={{
                fontSize: 16,
            ...
            >
              {data.map((datum) => (
                <Text style={styles.amount,{color:'#14213D',fontSize:18,marginTop: 16}}>{datum.euCharge}Fcfa</Text>
              ))}
            </Text>
          </View>
          <View style={{ flexDirection: "row" }}>
            ...
            {data.map((datum) => (
              <Text style={styles.amount}>{datum.euTotal}Fcfa</Text>
            ))}
          </View>
          <View style={{ flexDirection: "row" }}>
            ...
            <Text style={styles.amount}>
              {" "}
              {data.map((datum) => (
                <Text style={styles.amount}>{datum.euTax}Fcfa</Text>
              ))}
            </Text>
          </View>
        </View>
