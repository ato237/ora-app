...
let fullDate = `${day}.${month}.${year}`
  ...
  return (
    ...
    <ScrollView style={{backgroundColor:"#fff", padding:20}}>
        <TouchableOpacity onPress={()=>navigation.goBack()} style={{marginBottom:20}}><Ionicons name="arrow-back-outline" size={25}/> 
</TouchableOpacity>
    <View style={{justifyContent:'center',alignItems:'flex-start'}}>
        <View style={{flexDirection:'row', justifyContent:'center'}}>
        <Image style={{width:30,height:30}} source = {route.params.image}/>

<Text style={{fontSize:15, marginTop: 8,marginHorizontal:10, color:'grey'}}>{route.params.symbol}</Text>
        </View>

    <Text style={{fontSize:25, fontWeight:'bold', marginTop:15,color:'#14213D'}}>{route.params.wallet}</Text>
    <View>
    <Text style={{alignItems:'flex-end', justifyContent:'flex-end',marginTop:5}}>{fullDate}</Text>
